  ...
  return (
    <div className="flex flex-col h-full bg-gray-300 rounded-lg shadow-md">
      <div className="p-4 border-b flex justify-between items-center">
        <h2 className="text-xl font-bold">{showContacts ? "Contacts" : "Chats"}</h2>
        <button onClick={() => setShowContacts(!showContacts)}>
          {showContacts ? "Show Chats" : "Show Contacts"}
        </button>
          ...
          {contacts.map(contact => (
            ...
            >
              ...
              <div className="flex-1">
                <Link to={`/user/${contact._id}`}>
                  <p className="font-medium">{contact.username}</p>
                </Link>
                {contact.bio && <p className="text-sm text-gray-500 truncate">{contact.bio}</p>}
